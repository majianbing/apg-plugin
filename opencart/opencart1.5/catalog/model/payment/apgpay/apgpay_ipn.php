...
class ApgpayReturn extends Model{
    ...
    protected function ipn_success(){
        ...
        if ($order_info && $order_info['order_status_id'] == 0){
            return $this->model_checkout_order->confirm($this->orderId, $this->order_status_success, $comment,true);
        }else if($order_info){
            $this->model_checkout_order->update($this->orderId, $this->order_status_success, $comment,true);
        }
    ...
    protected function ipn_fail(){
        ...
        $order_info = $this->model_checkout_order->getOrder($this->orderId);
        if ($order_info && $order_info['order_status_id'] == 0){
            $this->model_checkout_order->confirm($this->orderId, $this->order_status_fail, $comment);
        }
    }
    ...
    protected function ipn_process(){
        ...
        $order_info = $this->model_checkout_order->getOrder($this->orderId);
        if ($order_info && $order_info['order_status_id'] == 0){
            $this->model_checkout_order->confirm($this->orderId, $this->order_status_process, $comment);
        }
    }
    ...
    protected function real_time_fail(){
        ...
        return '<script type="text/javascript">parent.location.href="' .
        $this->url->link('payment/apgpay/failure') . '"</script>';
    }
