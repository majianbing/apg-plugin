...
//        add_action( 'woocommerce_receipt_apgcreditcard', array( $this, 'apgcreditcard_receipt_page' ) );
    ...
    function notice_payment($order) {
        ...
        // Log the raw request if logging is enabled
        if ($this->settings['log'] === 'true') {
            $this->postLog(['raw_request' => $json], self::PUSH);
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            error_log('APG Payment Notice: Invalid JSON received');
            echo "fail";
            exit;
        }
        
        // Extract values from request
        $payment_id = isset($request['referenceNo']) ? sanitize_text_field($request['referenceNo']) : '';
        $order_number = isset($request['merOrderNo']) ? sanitize_text_field($request['merOrderNo']) : '';
        $order_currency = isset($request['payCurrency']) ? sanitize_text_field($request['payCurrency']) : '';
        $order_amount = isset($request['payAmount']) ? sanitize_text_field($request['payAmount']) : '';
        $payment_status = isset($request['respStatus']) ? sanitize_text_field($request['respStatus']) : '';
        $payment_details = isset($request['respDesc']) ? sanitize_text_field($request['respDesc']) : '';
        $back_signValue = isset($request['sign']) ? sanitize_text_field($request['sign']) : '';
        $order_notes = isset($request['remark']) ? sanitize_text_field($request['remark']) : '';
        
        // Validate required fields
        if (empty($order_number) || empty($payment_id) || empty($back_signValue)) {
            error_log('APG Payment Notice: Missing required fields');
            echo "fail";
            exit;
        }
        ...
        if (!$order) {
            error_log('APG Payment Notice: Order not found - ' . $order_number);
            echo "fail";
            exit;
        }
        ...
        // Calculate signature
        $befor_sign = $order_number . $payment_id . $order_currency . $payment_status . $this->settings['securecode'];
        $local_signValue = strtolower(hash('sha512', $befor_sign));
        
        // Log the notification if enabled
        if ($this->settings['log'] === 'true') {
            $this->postLog($request, self::PUSH);
        }
        ...
        if (strtoupper($local_signValue) == strtoupper($back_signValue)) {
            ...
            $order->update_status('failed', __($testorder . 'Invalid signature', 'apgpayment-creditcard-gateway'));
            echo "fail";
        }
        exit;
    }
